...
pub fn decimal_subtraction(a: Decimal, b: Decimal) -> StdResult<Decimal> {
    Ok(Decimal::from_ratio(
        Asset::checked_sub(a * DECIMAL_FRACTIONAL, b * DECIMAL_FRACTIONAL)?,
        DECIMAL_FRACTIONAL,
    ))
}
...
pub fn compute_short_reward_weight(premium_rate: Decimal) -> StdResult<Decimal> {
    let one = Decimal::one();
    let two = one + one;
    let e10 = 10000000000u128;
