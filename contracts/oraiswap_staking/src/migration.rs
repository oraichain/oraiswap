use cosmwasm_std::{to_vec, Api, CanonicalAddr, Order, StdResult, Storage};
...
pub fn migrate_asset_keys_to_lp_tokens(storage: &mut dyn Storage) -> StdResult<()> {
    ...
    for asset_key in asset_keys {
        if let Ok(pool_info) = read_pool_info(storage, &asset_key) {
            ...
            for staker in stakers {
                ...
                //         .save(&pool_info.staking_token, &rewards_bucket)?;
                //     // remove old key
